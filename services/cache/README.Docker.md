...
5. Verify
   `openssl verify -CAfile ca.crt fullchain.server.crt`
...
4. Cat Client certificate and CA cerficate to fullchain.client
   `cat client.crt ca.crt > fullchain.client.crt`

5. Verify
   `openssl verify -CAfile ca.crt fullchain.client.crt`
   `openssl s_client -connect 127.0.0.1:6379 -CAfile ca.crt -cert fullchain.client.crt -key client.key`
